...
int ftpclient_put(int sock_data, char *filename)//上传文件 
{
	...
	struct stat st;
	if (stat(filename, &st)<0)//将文件filename的信息保存在st结构中 
		return -1;

	sendfile(sock_data, fd, NULL, st.st_size);//将fd内容写入sock_data中 
	close(fd);
	return 0;
}
...
{
	char buf[MAXSIZE];
	sprintf(buf, "%s %s", cmd->code, cmd->arg);//格式化写入到buf中 
...
int ftpclient_login(int sock_ctl)//用户登录 
{
	struct command cmd;
	char user[256];
